...
- **Distributed as a GitHub release, installed by Homebrew or mise** (`github:timche/boswell`) or by downloading the binary. Two targets are built, `x86_64-unknown-linux-musl` and `aarch64-apple-darwin`, which are the machines this runs on; mise's github backend matches the OS and architecture in the asset name, so the triple stays in it. Anything else builds from source, which is a Rust toolchain the installing machine may not want but is at least a choice it can make. Not published to crates.io: nobody depends on this as a library, `cargo install` would mean a Rust toolchain and a source build on the target box, and crates.io versions can only be yanked, never deleted. The crate name is taken by an unrelated retry library anyway; the binary name is what matters and it is free.
- **A Homebrew tap of its own, [`timche/homebrew-tap`](https://github.com/timche/homebrew-tap), is the install route named first.** mise is being narrowed to language runtimes, and everything else on these machines comes from Homebrew; homebrew/core will not take a formula for a project nobody has heard of, so a personal tap is the only Homebrew route there is. The formula installs the release tarball rather than building from source, because the release already builds both targets and a source formula would put a Rust toolchain on every machine that installs it. The release workflow rewrites the formula and pushes it, so the tap cannot drift behind a release, and it authenticates with a deploy key scoped to that one repository instead of a personal token that would reach every repository the account can write. Generating the whole formula rather than patching its lines is what makes a rerun for an unchanged tag a no-op, which is what lets the update be dispatched by hand after a failure. No `service` block, though Homebrew would run one: the LaunchAgent and the systemd unit are owned by the dotfiles repository that also holds the config, and they carry paths and environment that belong to a machine rather than to a version.
- **Public repository.**
